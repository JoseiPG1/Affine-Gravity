...
The Poplaswki's metric tensor is given by
\begin{equation}
  \mathcal{T}_{\alpha\beta} = \left(\mathcal{B}_{\alpha}{}^{\sigma}{}_{\gamma} + \mathcal{A}_\alpha \delta^{\sigma}_{\gamma}\right)\left(\mathcal{B}_{\beta}{}^{\gamma}{}_{\sigma} + \mathcal{A}_\beta \delta^{\gamma}_{\sigma}\right)
\end{equation}
In the case where $\nabla_\alpha \mathcal{T}_{\beta\gamma} = 0$, we have three differential equations
\begin{align}
  \eta\dot{\eta} & = 0 & 2\eta^2g-h\psi^2 & = 0 & h\psi^2 - \psi\dot{\psi} & = 0
\end{align}
The solution of the above system allow us to writte down the complete set of affine functions that defines completly the affine connection
in terms of just one time depending function
\begin{align}
  g(t) & = \frac{\psi\dot{\psi}}{2\eta_0^2} & h(t) & = \frac{\dot{\psi}}{\psi} & \eta(t) & = \eta_0
\end{align}
