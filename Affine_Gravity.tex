...
\begin{itemize}
  ...
  \item Therefore, to have an affine scale factor, it is required that $\kappa \neq 0$.
  \item Under the special parametrization it is possible to solve the field equations.
\end{itemize}
...
\begin{itemize}
  ...
  \item Additionally, under the special parametrization, can also be solved numerically.
  \item The above its true only for $\kappa = 0$
...
For $\nabla_{[\sigma} \mathcal{R}_{\mu]\nu} = 0$ 
\begin{itemize}
  \item It is not possible to find analytically solutions (1 equation and two unknown functions $g(t)$ and $h(t)$).
  \item Under the special parametrization, its possible to solve the field equation numerically.
\end{itemize}

For  $\nabla_{[\sigma} \mathcal{S}_{\mu]\nu} = 0$ 
\begin{itemize}
  \item It is not possible to solve the field equation (1 equation for $g(t)$, $h(t)$, $\phi (t)$ and $\mathcal{V}(\phi)$).
  \item Under the special parametrization we have 1 equation for $a(t)$, $\phi (t)$ and $\mathcal{V}(\phi)$.
\end{itemize}
